...
try:
    ...
    cursor = conn.cursor()
    print("Running main sync query...")
    cursor.execute(query)
    rows = cursor.fetchall()
    print(f"Success! Found {len(rows)} picks.")
    if rows:
        print(f"Sample: {rows[0]}")
except Exception as e:
    print(f"FAILED with error: {e}")
    
    print("\nChecking individual table access...")
    tables = ["so_detail", "so_header", "item", "item_branch", "cust", "cust_shipto", "shipments_header", "pick_header", "pick_detail"]
    for table in tables:
        try:

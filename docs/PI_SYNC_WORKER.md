...
- `SYNC_API_KEY` and `CLOUD_API_URL` if the worker should use API mode instead of direct mirror writes
...
- If changes are detected, it syncs picks, work orders, and delivery KPIs into the mirror target.
